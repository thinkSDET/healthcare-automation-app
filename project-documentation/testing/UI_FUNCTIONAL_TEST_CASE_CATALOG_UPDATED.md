...
### E2E-020 — Admin Creates Doctor → Activation → Doctor Login

- **Type:** Workflow
- **Priority:** P0
- **Roles:** ADMIN → DOCTOR
- **Preconditions:** AdminA exists; unique Doctor test data available; local/dev password-reset flow is available
- **Depends:** AUTH-001, AUTH-010, AUTH-012, DR-002
- **Workflow Steps:**
  1. AdminA logs in and creates a Doctor with unique doctor code, license number and email.
  2. Verify the newly created Doctor is `INACTIVE` and the linked User account is `INVITED`.
  3. Attempt login with the newly created Doctor credentials before activation.
  4. Verify login is blocked while the account is `INVITED`.
  5. Start the local/dev Forgot Password flow for the Doctor account.
  6. Set a new password using the reset flow.
  7. Verify the account becomes `ACTIVE` and the Doctor becomes `ACTIVE`.
  8. Login as the Doctor with the new password.
- **Expected Result:** Admin-created Doctor completes the invitation/activation lifecycle; login is blocked before activation and succeeds after activation.
- **Automation Priority:** Critical
- **Suites:** Smoke, Sanity, Regression, Critical Workflow

### E2E-021 — Doctor Self-Registration → Admin Approval → Doctor Login

- **Type:** Workflow
- **Priority:** P0
- **Roles:** PUBLIC/DOCTOR → ADMIN → DOCTOR
- **Preconditions:** Unique Doctor registration data; AdminA can review registration requests
- **Depends:** AUTH-008, DR-002
- **Workflow Steps:**
  1. Register as DOCTOR with valid unique details.
  2. Verify the registration request is `PENDING`.
  3. Login as AdminA and open Doctor Registration Requests.
  4. Verify the new request is visible in the pending queue/count.
  5. Admin approves the request.
  6. Verify the request becomes `APPROVED` and the Doctor becomes `ACTIVE`.
  7. Login as the Doctor.
- **Expected Result:** A valid self-registration moves `PENDING → APPROVED → ACTIVE`, and the approved Doctor can log in.
- **Automation Priority:** Critical
- **Suites:** Smoke, Sanity, Regression, Critical Workflow

### E2E-022 — Doctor Self-Registration → Admin Rejection → Doctor Login With Rejection Notice

- **Type:** Negative / Workflow
- **Priority:** P0
- **Roles:** PUBLIC/DOCTOR → ADMIN → DOCTOR
- **Preconditions:** Unique Doctor registration data; AdminA can reject requests
- **Depends:** AUTH-008
- **Workflow Steps:**
  1. Register as DOCTOR with valid details.
  2. Verify the request is `PENDING`.
  3. AdminA opens the pending request and rejects it with a meaningful rejection reason.
  4. Verify the request becomes `REJECTED` and the reason is retained.
  5. Login as the rejected Doctor.
  6. Verify login is allowed.
  7. Verify the Doctor dashboard opens and a mandatory registration-update popup is displayed.
  8. Verify the popup contains the Admin's rejection reason.
- **Expected Result:** Rejection does not trap the Doctor outside the application; the Doctor can log in and is clearly informed why registration was rejected.
- **Automation Priority:** Critical
- **Suites:** Sanity, Regression, Critical Workflow, Negative

### E2E-023 — Rejected Doctor → Update & Resubmit → Admin Approval

- **Type:** Workflow
- **Priority:** P0
- **Roles:** DOCTOR → ADMIN → DOCTOR
- **Preconditions:** Doctor has an existing `REJECTED` registration request with a rejection reason
- **Depends:** E2E-022
- **Workflow Steps:**
  1. Login as the rejected Doctor.
  2. Verify the mandatory rejection popup appears.
  3. Verify the rejection reason is displayed.
  4. Click `Update & Resubmit`.
  5. Verify the registration form opens with existing Doctor information pre-filled.
  6. Correct/update the required information and submit.
  7. Verify the existing registration request returns to `PENDING`.
  8. Verify the previous rejection reason is cleared.
  9. AdminA opens the request and approves it.
  10. Verify the Doctor becomes `ACTIVE`.
  11. Login as the Doctor again.
- **Expected Result:** `REJECTED → PENDING → APPROVED → ACTIVE`; the same Doctor account/request is recovered without creating a duplicate identity.
- **Automation Priority:** Critical
- **Suites:** Smoke, Sanity, Regression, Critical Workflow

### E2E-024 — Rejected Doctor → Second Resubmission Cycle → Final Approval

- **Type:** Workflow
- **Priority:** P0
- **Roles:** DOCTOR → ADMIN → DOCTOR → ADMIN
- **Preconditions:** Doctor has a `REJECTED` request
- **Depends:** E2E-023
- **Workflow Steps:**
  1. Doctor resubmits after the first rejection.
  2. Admin rejects the resubmitted request again with a different meaningful reason.
  3. Verify the request is `REJECTED` with the new reason.
  4. Doctor logs in and verifies the popup displays the latest rejection reason.
  5. Doctor selects `Update & Resubmit`.
  6. Verify the form is pre-filled and the request returns to `PENDING`.
  7. Admin approves the request.
  8. Verify the Doctor becomes `ACTIVE`.
  9. Doctor logs in successfully.
- **Expected Result:** Multiple rejection/resubmission cycles are supported; the latest rejection reason replaces stale messaging and the lifecycle can eventually reach `ACTIVE`.
- **Automation Priority:** Critical
- **Suites:** Regression, Critical Workflow, Negative

### E2E-025 — Rejected Doctor → Resubmit With Same Email → No Duplicate Account

- **Type:** Negative / Data Integrity Workflow
- **Priority:** P0
- **Roles:** DOCTOR → ADMIN
- **Preconditions:** Doctor has an existing `REJECTED` registration request and User account
- **Depends:** E2E-022
- **Workflow Steps:**
  1. Doctor logs in after rejection.
  2. Selects `Update & Resubmit`.
  3. Keep the same registered email address and correct/update other information.
  4. Submit the registration.
  5. Verify the request becomes `PENDING`.
  6. Verify the Doctor can be reviewed again by Admin.
  7. Verify the resubmission does not create a second User account or a second registration identity for the same Doctor.
- **Expected Result:** Resubmission reuses the existing Doctor/User registration identity; no duplicate account is created because the Doctor uses the same email.
- **Automation Priority:** Critical
- **Suites:** Sanity, Regression, Critical Workflow, Negative

### E2E-026 — Already Reviewed Doctor Request Cannot Be Approved or Rejected Again

- **Type:** Negative / State Transition Workflow
- **Priority:** P0
- **Roles:** ADMIN
- **Preconditions:** One request has been approved; another request has been rejected
- **Depends:** E2E-021, E2E-022
- **Workflow Steps:**
  1. Admin approves a `PENDING` request.
  2. Attempt to approve the same request again.
  3. Attempt to reject the same already-approved request.
  4. Use a separate `REJECTED` request and attempt to approve it without resubmission.
  5. Attempt to reject the same already-rejected request again.
- **Expected Result:** Already reviewed requests cannot be reviewed again; only a valid `PENDING` request can transition through Admin approval/rejection.
- **Automation Priority:** Critical
- **Suites:** Regression, Critical Workflow, Negative

### E2E-027 — Existing Doctor Conflict Blocks Registration Approval

- **Type:** Negative / Data Integrity Workflow
- **Priority:** P0
- **Roles:** ADMIN → DOCTOR
- **Preconditions:** Existing Doctor with the same email or license number; separate Doctor registration request is `PENDING`
- **Depends:** DR-002
- **Workflow Steps:**
  1. Create/ensure an existing Doctor with a known email or license number.
  2. Create a separate Doctor registration request that conflicts with that existing Doctor.
  3. Admin opens the pending request and selects Approve.
  4. Verify approval is blocked with the duplicate Doctor message.
  5. Verify no duplicate Doctor record is created.
  6. Verify the registration request does not incorrectly become `APPROVED`.
- **Expected Result:** Approval is prevented when email or license number conflicts with an existing Doctor; application data remains consistent.
- **Automation Priority:** Critical
- **Suites:** Regression, Critical Workflow, Negative

### E2E-028 — Doctor ACTIVE → INACTIVE → ACTIVE Controls Downstream Availability

- **Type:** Workflow
- **Priority:** P1
- **Roles:** ADMIN → DOCTOR/PATIENT
- **Preconditions:** Existing ACTIVE Doctor; PatientA exists
- **Depends:** DR-006, APPT-003, APPT-005
- **Workflow Steps:**
  1. Verify the Doctor is `ACTIVE` and available for the applicable clinical flow.
  2. Admin changes the Doctor status to `INACTIVE`.
  3. Verify the Doctor status is reflected as `INACTIVE`.
  4. Attempt to schedule a new appointment using the inactive Doctor.
  5. Verify the business flow blocks scheduling with the inactive Doctor.
  6. Admin changes the Doctor status back to `ACTIVE`.
  7. Verify the Doctor becomes available again for the applicable clinical flow.
- **Expected Result:** Doctor status has a real business effect: `ACTIVE → INACTIVE` blocks new clinical use where enforced, and `INACTIVE → ACTIVE` restores availability.
- **Automation Priority:** High
- **Suites:** Sanity, Regression, Critical Workflow, Negative

### E2E-029 — Patient Sees Only ACTIVE Doctors

- **Type:** Workflow / Ownership of Availability
- **Priority:** P0
- **Roles:** ADMIN → PATIENT
- **Preconditions:** At least one ACTIVE Doctor and one INACTIVE Doctor exist
- **Depends:** DR-006
- **Workflow Steps:**
  1. Admin ensures DoctorA is `ACTIVE` and DoctorB is `INACTIVE`.
  2. Login as PatientA.
  3. Open the patient-facing Doctor selection/list flow.
  4. Verify DoctorA is available.
  5. Verify DoctorB is not exposed as an available Doctor.
  6. If a direct Doctor-detail route is reachable, attempt to open the INACTIVE Doctor.
- **Expected Result:** Patient-facing Doctor consumption exposes only ACTIVE Doctors; inactive Doctors are hidden/unavailable.
- **Automation Priority:** Critical
- **Suites:** Smoke, Sanity, Regression, Critical Workflow, Negative
...
`AUTH-004, AUTH-008, AUTH-010, AUTH-014, DASH-002, DASH-005, PAT-006, DEP-001, EMG-001, MEDP-001, DOC-007, DR-005, DR-006, APPT-010, APREQ-011, RX-005, REFILL-006, ORD-004, ORD-009, PAY-003, PHARM-006, PHARM-007, INV-005, INV-008, REPL-003, AUD-003, AUD-006, LAB-006, PORTAL-002, PORTAL-006, E2E-002, E2E-003, E2E-006, E2E-007, E2E-009, E2E-011, E2E-012, E2E-013, E2E-014, E2E-017, E2E-022, E2E-024, E2E-026, E2E-027, E2E-028`
...
| E2E | Cross-module business journeys | E2E-001–029 | Yes |
...
| Total test cases | **264** (+10 Doctor lifecycle E2E workflows) |
| Cross-module E2E workflows | **29** (`E2E-001`–`E2E-029`) |
| Smoke suite IDs | **90** |
| Sanity suite IDs | **130** |
| Regression suite IDs | **264** (all) |
| Negative/RBAC/ownership-focused IDs | **81** |
| Critical Workflow suite | `E2E-001`–`E2E-029` (business journeys), plus module workflow-tagged cases |
...
| Doctor onboarding — Admin create / activation / login | Yes | E2E-020 |
| Doctor self-registration — approve / login | Yes | E2E-021 |
| Doctor self-registration — reject / login / rejection reason | Yes | E2E-022 |
| Doctor rejection — resubmit / approve | Yes | E2E-023 |
| Doctor rejection — repeated rejection / resubmission | Yes | E2E-024 |
| Doctor resubmission — same email / no duplicate account | Yes | E2E-025 |
| Doctor registration state-transition protection | Yes | E2E-026 |
| Doctor duplicate conflict / approval protection | Yes | E2E-027 |
| Doctor ACTIVE / INACTIVE downstream availability | Yes | E2E-028 |
| Patient consumes only ACTIVE Doctors | Yes | E2E-029 |
...
```text
AUTH / PAT / DR
  → Doctor onboarding → E2E-020 / E2E-021 / E2E-022 / E2E-023 / E2E-024 / E2E-025 / E2E-026 / E2E-027 / E2E-028 / E2E-029
...
- E2E section expanded to 29 distinct business journeys, including 10 Doctor lifecycle/onboarding journeys (E2E-020–E2E-029).
